function [ returnedData, success ] = sendNumericArray_fastIMCOps_yesADC_noCompsLimit( fileHandle, numArray, numSentBytesAtATime, repeatAvg, numComparisions_total  )
% display( mfilename )
...
%     (4*repeatAvg)*N1*N2 + stopByte
        numScanouts = ceil( numComparisions_total/31 ) ; 
    expect_size = ( ( 4 * repeatAvg ) + numScanouts ) + 1 ; % 1 byte is the scanout data and rest are for adcread
