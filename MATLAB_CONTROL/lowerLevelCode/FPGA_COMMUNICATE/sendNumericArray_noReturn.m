function returnedData = sendNumericArray_noReturn( fileHandle, numArray, numSentBytesAtATime )
% display( mfilename )
% fileHandle is actually a struct, fileHandle.s is a serialport object
%%
maxWaitTime = 5 ; % in seconds
%% SENDING numArray
...
%% NOW TO WAIT FOR THE STOPBYTE: 127
    ...
    while fnOutputByte~= 127
        while( fileHandle.s.NumBytesAvailable < 1 )
            ...
            if( timeElapsed > maxWaitTime )
                %killAllSuppliesAndPause() ;
                %fprintf('Stop byte has not been received and the max wait time of %d s has elapsed\n',maxWaitTime);
                %break;
            end
        end
        tic ;
