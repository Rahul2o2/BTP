...
function fpgaReceivedFile_blockSplit = uart_blockSplit( fpgaReceivedFile_name )
% display( mfilename )
% fileInPath is the fpgaReceivedFile_name of the .txt file in ./
fpgaReceivedFile              = [  fpgaReceivedFile_name '_receive.txt' ] ;
...
fpgaReceivedFile_temp_1     = [  fpgaReceivedFile_name '_receive_temp_1' ] ;
fpgaReceivedFile_temp_2     = [  fpgaReceivedFile_name '_receive_temp_2' ] ;
...
% system( perlCommand ) ;
%% NEW COMMAND
bashCommandFile = sprintf( './%s.bash', fpgaReceivedFile_name ) ;
...
%system( sprintf( '"C:\\Program Files (x86)\\Git\\bin\\bash" %s', bashCommandFile ) ) ;
