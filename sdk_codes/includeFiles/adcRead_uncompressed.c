...
//		static int SysMonFractionToInt(float FloatNum);
		double check_VPVN, check_AUX00, check_VPVN_map, check_AUX00_map;
		u16 adcOut_VPVN[max_XADCrepeatAvg];
		u16 adcOut_AUX00[max_XADCrepeatAvg];
		XADCvolt XADCv;
		XADCv.adcOut_VPVN[max_XADCrepeatAvg];
		XADCv.adcOut_AUX00[max_XADCrepeatAvg];
		u8 SeqMode;
		u16 ExtVolRawData;
...
//		{ //wait until EOS activated
		...
		for ( int iter=0; iter<repeatAvg; iter++)
		{
			...
			XADCv.adcOut_VPVN[iter]=ExtVolRawData* 0.244 * 0.001;
			...
			ExtVolRawData=ExtVolRawData>>4;
			XADCv.adcOut_AUX00[iter]=ExtVolRawData* 0.244 * 0.001;
