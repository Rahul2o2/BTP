...
//
////	X_Batch = 8; //Make it one as of now.
//
...
////    uint16_t** M_KSize = (uint16_t**)calloc(CNV_Layers,sizeof(uint16_t*)); //Kernals
////    uint16_t** M_Padding = (uint16_t**)calloc(CNV_Layers,sizeof(uint16_t*)); //Padding
////    uint16_t** M_Stride = (uint16_t**)calloc(CNV_Layers,sizeof(uint16_t*));  //Stride
////    uint16_t** M_Dilation = (uint16_t**)calloc(CNV_Layers,sizeof(uint16_t*)); //Dilation
////    for(uint16_t i = 0; i < CNV_Layers; i++){
...
////    }
////
////    char *maxParamsBuf = (char *)calloc(32*CNV_Layers, sizeof(char));
...
//
//
//     //First Layer Convolution + Relu + MaxPool + Flatten here.
//    uint16_t Layer_ID = 0;
//    #pragma region
//
//    //Convolution.
...
//	uint16_t chunks = ceil((float)(X_out_h1*X_out_w1)/(float)WordLines[0]); //Input Features/Wordlines
...
//				Ref[c][i][n] = (float*)calloc(WordLines[0],sizeof(float)); //need to make it layer specific
//		}
//	}
...
//			RefB[c][n] = (float*)calloc(WordLines[0],sizeof(float)); //need to make it layer specific
//		}
//	}
...
//	getRefForLayer(imageBuffer_rdata,offset_bytes,chunks,8,4,WordLines[0],Ref,RefB,RefMax);
//
...
//
//	// EXTRA STUFF FOR WRITING OUT MAC STARTS HERE
...
////	uint16_t W_outHeight = ceil((float)W_outWidth / (float)WordLines[Layer_ID]); // Number of WordLines sized chunks required
////
...
////						for(uint16_t xb = 0; xb < X_BitMat_Bits; xb+=XBL){
////
////	float**** MAC_Out1 = (float****)calloc(X_BitMat_Batch1,sizeof(float***));
////	for(uint16_t b = 0; b < X_BitMat_Batch1; b++){
////		MAC_Out1[b] = (float***)calloc(W_BitMat_Batch1,sizeof(float**));
////		for(uint16_t c = 0; c < W_BitMat_Batch1; c++){
////			MAC_Out1[b][c] = (float**)calloc(X_out_h1,sizeof(float*));
////			for(uint16_t h = 0; h < X_out_h1; h++){
////				MAC_Out1[b][c][h] = (float*)calloc(X_out_w1,sizeof(float));
////			}
////		}
////	}
//	// EXTRA STUFF FOR WRITING OUT MAC ENDS HERE
//
//	mode_type mode;
//		//	mode.mode=mode_OCC;
//		mode.mode=mode_ICQ;
...
//		int mini_batch_size_lim=1000;//mini batch size search
...
////		currObj = convolve(X_Mat,X_Batch,X_Channels,X_Height,X_Width,X_Padding[Layer_ID][0],X_Stride[Layer_ID][0],W_Mat,W_Batch[Layer_ID],W_Channels[Layer_ID],W_Height[Layer_ID],W_Width[Layer_ID],
////					W_Padding, W_Stride, WordLines[Layer_ID], XDL, XBL, WDL, WBL, IMC, Bias,Ref,RefB,RefMax,CNV_Out1,mode,slopeSetting,currObj);
////	currObj = convolve_XADC(X_Mat,X_Batch,X_Channels,X_Height,X_Width,X_Padding[Layer_ID][0],X_Stride[Layer_ID][0],W_Mat,W_Batch[Layer_ID],W_Channels[Layer_ID],W_Height[Layer_ID],W_Width[Layer_ID],
////				W_Padding, W_Stride, WordLines[Layer_ID], XDL, XBL, WDL, WBL, IMC, Bias,Ref,RefB,RefMax,CNV_Out1,mode,slopeSetting,currObj);
//	// method2: all at once (ends)
...
//
//
//	float CNV_Out1_see[X_BitMat_Batch1][W_BitMat_Batch1][X_out_h1][X_out_w1] ;
//	    for(uint16_t b = 0; b < X_BitMat_Batch1; b++){
//	        for(uint16_t c = 0; c < W_BitMat_Batch1; c++){
//	            for(uint16_t h = 0; h < X_out_h1; h++){
//	            	for(uint16_t w = 0; w < X_out_w1; w++){
//	            		CNV_Out1_see[b][c][h][w]=CNV_Out1[b][c][h][w];
//	            	}
//	            }
//	        }
//	    }
...
//	u32 size=X_BitMat_Batch1*W_BitMat_Batch1*X_out_h1*X_out_w1*sizeof(CNV_Out1[0][0][0][0]);
//	u32 SourceAddress;
//
//
//		float size_params[]={X_BitMat_Batch1,W_BitMat_Batch1,X_out_h1,X_out_w1};
//		int size_params_size=sizeof( size_params ) ;
//		FRESULT rc=WriteFile_custom(FileName_outI, size_params_size, size_params);
//				rc=WriteFile_fseekappend(FileName_outI, size, CNV_Out1_see);
//	}
//
...
////    #pragma region
////
////    //perform MaxPool here. Replaces CNV_Out
////    uint16_t pool_paddedHeight = X_out_h1 + 2 * M_Padding[Layer_ID][0]; //Padding is zero
////    uint16_t pool_paddedWidth = X_out_w1 + 2 * M_Padding[Layer_ID][1]; // Padding is zero
////    uint16_t Pool_out_h1 = ((pool_paddedHeight - M_KSize[Layer_ID][0]) / M_Stride[Layer_ID][0] + 1 ); //Pool window is 2
////    uint16_t Pool_out_w1 = ((pool_paddedWidth - M_KSize[Layer_ID][1]) / M_Stride[Layer_ID][0] + 1 ); //Pool window is 2
////
////    float**** POOL_Out1 = (float****)calloc(X_BitMat_Batch1,sizeof(float***));
////    for(uint16_t b = 0; b < X_BitMat_Batch1; b++){
////        POOL_Out1[b] = (float***)calloc(W_BitMat_Batch1,sizeof(float**));
////        for(uint16_t c = 0; c < W_BitMat_Batch1; c++){
...
////        }
////    }
////    MaxPooling(CNV_Out1, X_Batch,W_Batch[Layer_ID],X_out_h1,X_out_w1, M_KSize[Layer_ID][0], M_KSize[Layer_ID][1],M_Padding[Layer_ID][0],
////                M_Stride[Layer_ID][0],POOL_Out1);
////
////    #pragma endregion
//
//    //Flatten
//    #pragma region
//    //Create memory for flattened array.Unpadded array.
//    uint16_t features1 = W_Batch[Layer_ID]*X_out_h1*X_out_w1;
//    float** Flatten_Out1 = (float**)calloc(X_BitMat_Batch1,sizeof(float*));
//    for(uint16_t b = 0; b < X_BitMat_Batch1; b++){
//        Flatten_Out1[b] = (float*)calloc(features1,sizeof(float));
//    }
//
//    flatten(CNV_Out1,X_Batch,W_Batch[Layer_ID],X_out_h1,X_out_w1,Flatten_Out1);
//
//    float Flatten_Out1_see[X_BitMat_Batch1][features1] ;
//	for(uint16_t b = 0; b < X_BitMat_Batch1; b++){
//		for(uint16_t c = 0; c < features1; c++){
//			Flatten_Out1_see[b][c] = Flatten_Out1[b][c];
//		}
//	}
//    #pragma endregion
//
...
//	u32 size=X_BitMat_Batch1*features1*sizeof(Flatten_Out1_see[0][0]);
//	u32 SourceAddress;
//	{
//		float size_params[]={X_BitMat_Batch1,features1};
//		int size_params_size=sizeof( size_params ) ;
//		FRESULT rc=WriteFile_custom(FileName_outR, size_params_size, size_params);
//		        rc=WriteFile_fseekappend(FileName_outR, size, Flatten_Out1_see);
...
////    //Extract Reference from buffer
////    //Create memory for ref array = chunks * 2 * na(8) * nwl
////    offset_bytes = chunks * 20 * WordLines[0]; //Chunks[:] * 2 * Na (8 + 4) * WordLines[:] of previous layer
////
////	chunks = ceil((float)features1/(float)WordLinesL[0]); //Input Features/Wordlines
...
////
////    //Call linear here.
////	currObj = linear_NotOnChip(Flatten_Out1,X_Batch,features1,W_Mat_Lin,W_Batch[Layer_ID],Bias,WordLinesL[0],
